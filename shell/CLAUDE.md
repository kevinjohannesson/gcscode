...
| Actor class | Role                | Kind                  | Identity                  | Model                                          | Secondary model   | Targets         | Trigger                              | Routing condition | Verdicts                         | Character                                                          | Header                                                       | Re-review header                                                                | Prompt template                                                           |
| ----------- | ------------------- | --------------------- | ------------------------- | ---------------------------------------------- | ----------------- | --------------- | ------------------------------------ | ----------------- | -------------------------------- | ------------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| reviewer    | Spec-compliance     | per-task              | `gcscode-spec-compliance[bot]` | Claude Sonnet 4.6                              | —                 | feature-PR      | After each task commit               | always            | `--comment`, `--request-changes` | Verify implementation matches the task's spec slice                | `## Spec-compliance review — task <N> — Claude Sonnet 4.6`   | `## Spec-compliance review — task <N> (re-review of <SHA>) — Claude Sonnet 4.6` | `superpowers:subagent-driven-development/spec-reviewer-prompt.md`         |
| reviewer    | Code-quality        | per-task              | `gcscode-code-quality[bot]`    | Claude Sonnet 4.6                              | —                 | feature-PR      | After spec-compliance passes         | always            | `--comment`, `--request-changes` | Code quality, idioms, edge cases                                   | `## Code-quality review — task <N> — Claude Sonnet 4.6`      | `## Code-quality review — task <N> (re-review of <SHA>) — Claude Sonnet 4.6`    | `superpowers:subagent-driven-development/code-quality-reviewer-prompt.md` |
| reviewer    | Final cross-cutting | cross-cutting         | `gcscode-final-review[bot]`    | Claude Opus 4.7                                | —                 | feature-PR      | End of iteration                     | always            | `--request-changes`, `--approve` | Cross-cutting concerns missed at per-task level                    | `## Final cross-cutting review — Claude Opus 4.7`            | `## Final cross-cutting review (re-review of <SHA>) — Claude Opus 4.7`          | `superpowers:requesting-code-review/code-reviewer.md`                     |
| reviewer    | Red-team            | per-artifact          | `gcscode-red-team[bot]`        | Claude Opus 4.7                                | Claude Sonnet 4.6 | spec-PR, ADR-PR | Automatic on PR open                 | always            | `--comment` only (v1)            | Premise challenger + consistency reviewer                          | `## Red-team review — <spec or ADR> — Claude Opus 4.7`       | `## Red-team review — <spec or ADR> (re-review of <SHA>) — Claude Opus 4.7`     | `.claude/reviewer-prompts/red-team.md`                                    |
| reviewer    | Spec-quality        | per-artifact          | `gcscode-spec-quality[bot]`    | Claude Sonnet 4.6                              | —                 | spec-PR, ADR-PR | Automatic on PR open                 | always            | `--comment` only (v1)            | Document structure + within-document consistency + link mechanics  | `## Spec-quality review — <spec or ADR> — Claude Sonnet 4.6` | `## Spec-quality review — <spec or ADR> (re-review of <SHA>) — Claude Sonnet 4.6` | `.claude/reviewer-prompts/spec-quality.md`                                |
| reviewer    | Extension architect | per-artifact          | `gcscode-extension-architect[bot]` | Claude Opus 4.7                                | —                 | spec-PR, ADR-PR | Automatic on PR open (subject to routing) | heuristic: PR touches extension-architecture concerns | `--comment` only (v1)            | Principal-level architect on extensible hosts; reviews host↔extension boundary, lifecycle, isolation, IPC, discovery, conflicts, performance | `## Extension-architect review — <spec or ADR> — Claude Opus 4.7` | `## Extension-architect review — <spec or ADR> (re-review of <SHA>) — Claude Opus 4.7` | `.claude/reviewer-prompts/extension-architect.md`                          |
| respondent  | Respondent          | per-followup-commit (new enum value; extends `{per-task, cross-cutting, per-artifact}`) | `gcscode-respondent[bot]` | Claude Sonnet 4.6 | — | spec-PR, ADR-PR (v1 scope; review-discussion-loop-v1's Future iterations contemplates feature-PR extension) | After each Code-review-followup commit | always | `--comment` only (advisory; not a review verdict) | Documents controller's per-finding dispositions; documents what was addressed, deferred, routed, or noted | `## Respondent — re commit <SHA> — to <reviewer role> review by <reviewer model>` | — (the respondent does NOT re-review its own prior posts; each followup commit triggers a new respondent post with the standard header above, NOT a re-review of a prior respondent post) | `.claude/reviewer-prompts/respondent.md` |
...
Extension architect's re-review checks each tagged-blocking question for substantive disposition; silent dispositions are re-flagged. Spec: [`docs/specs/2026-05-17-reviewer-routing-and-domain-expert.md`](docs/specs/2026-05-17-reviewer-routing-and-domain-expert.md).
