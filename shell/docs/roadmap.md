...
- [ ] **Superpowers baseline reviewers on spec/ADR PRs?** Currently the three superpowers baseline reviewer roles (spec-compliance, code-quality, final cross-cutting) have `targets: feature-PR` only — they don't fire on spec/ADR PRs, where only red-team runs. Open question whether some or all should also apply to spec/ADR PRs (e.g., a "spec-prose-quality" counterpart to code-quality that checks internal consistency / clarity of the spec text itself, distinct from red-team's premise-challenger + consistency-reviewer mandate). Trigger: an iteration where red-team's organic critique misses something a baseline-style reviewer would have caught. Surfaced during PR #4 (reviewer-role-design-conventions, 2026-05-14) — the first organic spec-PR went well with red-team alone, but the question of "both" reviewer flavors is worth a focused brainstorm before the next non-trivial spec-PR.
