...
- **Edit the relative-paths spec's "Divergence" subsection.** Per CLAUDE.md "Specs as historical record," substantive edits to merged specs are out of scope. The spec's "divergence" framing was true at the time of writing; this iteration resolves the queued question by revising the convention, not by editing the predecessor.
...
- **The carve-out's wording may not be tight enough.** "Binary grep-able rules" is intentionally informal. Future iterations may need a more rigorous criterion (e.g., "decidable in constant time on the review body's text"). Trigger to revisit: a tripwire whose binary-rule status is ambiguous and the controller can't tell which threshold to use.
