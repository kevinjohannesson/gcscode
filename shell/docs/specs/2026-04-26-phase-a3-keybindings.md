...
| `executeCommand` from non-extension code                               | Both `host.executeCommand` and `registry.executeCommand` (one shared `execute()` helper) | Aligned — in VS Code, the keybinding system is editor-core code that calls `executeCommand` without an extension identity; our dispatcher is shell-core code with the same shape. The two methods share one internal helper; only the error-attribution string differs (`plugin "<id>"` vs `host`). The dispatcher is intentionally NOT wrapped in a fake "shell plugin" — that would invent a `Plugin` identity for what is really shell-internal machinery. |
