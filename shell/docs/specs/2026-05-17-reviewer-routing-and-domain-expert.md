...
5. **Blocking-question discipline (architectural-intent questions only).** A subset of domain expert's "Open questions" — those tagged architectural-intent by the reviewer (with a `[blocking]` prefix or **bold-leading-text**) — are blocking-by-default: each tagged question requires an explicit substantive disposition before re-review can verify it as addressed. Other domain-expert open questions (minor clarification, future-iteration suggestions, devil's-advocate flags) remain informational. Uses existing respondent disposition vocabulary; no new infrastructure. Silent dispositions on tagged questions ("noted, no current action — we'll consider it") are re-flagged.
...
- **Silent-disposition pattern tripwire**: if controller's respondent posts use bare `noted, no current action — <generic placeholder>` on domain expert's tagged-blocking open questions across **N=2 spec/ADR PRs**, the blocking-question discipline isn't holding. Response: revise the respondent prompt template to reject silent dispositions for questions tagged blocking-by-default (the disposition vocabulary becomes stricter for these).
- **Architectural-intent tagging failure-mode tripwire (NEW, per Opus's round-2 re-review)**: the architectural-intent tagging mechanic has two failure modes. **Tag-zero**: if domain expert produces a substantive review with ANY open questions but tags ZERO as architectural-intent across **N=3 spec/ADR PRs** that contain non-trivial extension-architecture content, the reviewer is avoiding accountability for real architectural questions. **Tag-everything**: if domain expert tags ALL open questions as architectural-intent across **N=2 spec/ADR PRs**, the reviewer is restoring the original blocking-all-questions problem the architectural-intent narrowing was meant to fix. Response (tag-zero): sharpen the prompt template's framing on what qualifies as architectural-intent and add explicit examples. Response (tag-everything): clarify the CLASS boundary in the prompt template + add the disposition-theater failure mode as an explicit warning. Both tripwires are pattern-across-N; both align with CLAUDE.md condition (iii).
