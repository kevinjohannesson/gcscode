# Reviewer routing layer + Domain expert v1 (Extension architect)

(Iteration title is "Reviewer routing layer + Domain expert v1 (Extension architect)" to capture both the routing layer that ships AND the first domain-expert-class role being instantiated. "Domain expert" is the category; "Extension architect" is the specific role identifier used throughout the rest of the spec — App name `gcscode-extension-architect`, registry role name `Extension architect`, slug `extension-architect`.)

**Slug:** reviewer-routing-and-domain-expert (matches the spec filename; the iteration adds **domain-expert-class** reviewing capability via routing, with **Extension architect** as the v1 instance — future iterations may add additional domain-expert-class roles using the same routing-layer substrate)
**Iteration on the agentic-team track:** eighteenth.
**Type:** combined architecture iteration. Closes queued #4 (reviewer routing layer) from [`docs/specs/2026-05-16-agentic-team-debt-clearing-v1.md`](2026-05-16-agentic-team-debt-clearing-v1.md) AND adds the first reviewer role that uses routing (Extension architect on extension architecture). The two are bundled because routing-layer architecture needs a concrete user to validate against, and the Extension architect needs routing to avoid noise on PRs outside its scope.
**No bootstrap exceptions.** Spec ships via spec-PR workflow; implementation lands per the post-merge implementation convention. Operational prerequisite: user creates one new GitHub App (`gcscode-extension-architect`).

## Context

Two threads converge into this iteration.

**Thread 1 — Reviewer routing layer (queued #4).** From `agentic-team-debt-clearing-v1.md`: "Reviewer routing layer — load-bearing when a 4th reviewer role arrives." Today all existing reviewer roles default to "always dispatch" on their trigger (per-task reviewers on each commit; red-team + spec-quality on every spec/ADR PR). This works because every existing reviewer has a mandate that applies to every PR in its target class. A 4th role with a NARROWER mandate breaks the assumption — it doesn't make sense to dispatch an Extension architect on every spec/ADR PR; many PRs have nothing to do with the domain. The routing question becomes: how does the controller decide which reviewers to dispatch on which PRs?

**Thread 2 — Extension architect reviewer.** Drafted in an earlier brainstorm session (the user's prior draft), refined in feasibility-check against the current architecture. The role: principal-level architect on extensible hosts who reviews specs/ADRs/plans touching the host↔extension boundary (API surface, lifecycle, isolation, IPC, discovery, conflict resolution, performance characteristics). The persona is VSCode/Atom/Eclipse-flavored — appropriate for gcscode because gcscode is explicitly "VS Code-style" per `shell/CLAUDE.md`. The role is **scoped**: it does not review every PR; only PRs that touch its domain.

The two threads are mutually validating:

- Without routing, the Extension architect would dispatch on every spec/ADR PR and produce "out of scope, defer" responses on most — wasting tokens AND polluting the PR with noise from a reviewer that has nothing of substance to say.
...
Combining them: the routing layer ships AND the Extension architect ships as its first user. The routing layer's heuristic + escape-valve pattern is validated end-to-end against the Extension architect's real scope decisions.

## Why not the bigger version

The bigger version would include:

- **Ship routing layer alone, defer Extension architect.** **Smaller wedge:** routing alone with hypothetical 4th-reviewer language. **Bigger wedge:** speculative routing design without a concrete user to validate against. YAGNI in the inverse direction — routing layer needs a real first instance to design correctly.
- **Ship Extension architect with always-dispatch.** **Smaller wedge:** no routing changes; Extension architect dispatches on every spec/ADR PR like red-team and spec-quality. **Bigger wedge:** token waste + PR noise on PRs unrelated to extension architecture. Defers the routing-layer iteration. The combined design fixes both concerns at once.
- **Ship Extension architect with verdict promotion** (`--request-changes` blocking power). **Smaller wedge:** v1 stays `--comment` only, consistent with red-team and spec-quality v1. **Bigger wedge:** verdict-promotion has its own design space (override mechanics, escalation paths, blocking-merge semantics under auto-merge) that's distinct from the routing-layer question. Future iteration if needed; v1 advisory.
- **Ship multiple domain experts** (extension architecture + frontend + security + …). **Smaller wedge:** single Extension architect (extension architecture only). **Bigger wedge:** speculative scope expansion; gcscode has one domain that's load-bearing right now (extension architecture). Other domains added when their need surfaces. The routing-layer pattern is reusable for the second Extension architect when it ships.
- **Ship "out-of-scope skip" exit pattern for all reviewers** (red-team, spec-quality, per-task, Extension architect). **Smaller wedge:** out-of-scope skip applies only to roles whose dispatch condition is heuristic-conditional (Extension architect in v1). Existing always-dispatch roles (red-team, spec-quality, per-task reviewers) stay always-dispatch and always-substantive. **Bigger wedge:** generalizes the escape valve to every reviewer, which is over-reach for v1 because the existing roles' mandates DO apply universally.
...
This iteration ships: registry column for routing + escape-valve output form + Extension architect agent file + prompt template + GitHub App + CLAUDE.md updates. Direct master commits after merge.
...
3. **Out-of-scope skip exit pattern.** Any reviewer whose `Routing condition` is `heuristic` can return a one-line "out-of-scope skip" response if the controller's heuristic was wrong and the reviewer determines the PR is actually out-of-scope at review time. The controller treats a skip as a no-op: it doesn't count toward Gate 3b (irrelevant since this reviewer isn't in the gate), doesn't trigger the followup loop, and doesn't request re-review on subsequent commits — **except** when a `Code-review-followup:` commit materially changes scope (heuristic re-evaluation may fire a fresh dispatch then).
4. **Extension architect reviewer role.** New role added to the agentic-actor registry: principal-level architect on extensible hosts, scoped to host↔extension boundary concerns. Persona is VSCode/Atom/Eclipse-flavored AND includes gcscode-specific awareness (Svelte 5 host, contribution-surface model, Disposable lifecycle). Free to push back on past gcscode architectural choices, not shackled to defending them.
5. **Blocking-question discipline (architectural-intent questions only).** A subset of Extension architect's "Open questions" — those tagged architectural-intent by the reviewer (with a `[blocking]` prefix or **bold-leading-text**) — are blocking-by-default: each tagged question requires an explicit substantive disposition before re-review can verify it as addressed. Other extension-architect open questions (minor clarification, future-iteration suggestions, devil's-advocate flags) remain informational. Uses existing respondent disposition vocabulary; no new infrastructure. Silent dispositions on tagged questions ("noted, no current action — we'll consider it") are re-flagged.
6. **Output format aligned to existing 4-section convention.** Extension architect produces the same 4-section structure as red-team and spec-quality (Premises challenged / Drift from existing decisions / Open questions / Summary), adapted to its mandate. NOT the 5-section draft format from the prior brainstorm — alignment with existing conventions wins over inherited draft structure.
7. **Operational prerequisite: 1 new GitHub App.** User creates `gcscode-extension-architect`, provides appId + installationId, saves PEM at the canonical default path. Spec specifies the App-creation steps.
8. **CLAUDE.md additions.** Registry table gets new column + new row. Verdict-permission table gets new row for Extension architect. "Auto-dispatch on spec/ADR PRs" subsection updated. PR-template footer text updated (drop "Extension architect, when they exist" since the role exists now). Two new CLAUDE.md subsections: "Routing layer discipline" (which embeds the Out-of-scope skip exit pattern as part of its content) + "Blocking-question discipline."
...
- **Verdict promotion for Extension architect.** Extension architect stays `--comment` only in v1, consistent with red-team and spec-quality v1. Future iteration if the advisory mandate proves insufficient.
...
- **Extension architect in Gate 3b.** The merge gate stays `gcscode-red-team count >= 2 + gcscode-spec-quality count >= 1`. Extension architect is advisory; its absence doesn't block merge, its presence doesn't enable merge. Workflow unchanged.
- **Per-task Extension architect dispatch on feature-PRs.** Extension architect dispatches on spec/ADR PRs only (matching red-team and spec-quality). Feature-PRs continue to use the existing per-task reviewer flow.
...
- `heuristic: <one-line description>` — dispatch conditionally. The description tells the controller what to check. v1 has exactly one heuristic-conditional role: Extension architect, with `heuristic: PR touches extension-architecture concerns`.
...
### Extension architect's heuristic

The heuristic for Extension architect (the only heuristic-conditional role in v1):

**`heuristic: PR touches extension-architecture concerns`** — concretely, the controller dispatches Extension architect if ANY of:
...
**Skip response header:** `## <Role> review — out-of-scope skip — <model>` (e.g., `## Extension-architect review — out-of-scope skip — Claude Opus 4.7`)
...
### Extension architect persona + scope
...
**Scope (what the Extension architect reviews):**
...
Extension architect's output uses the same 4-section structure as red-team and spec-quality, adapted to its mandate:
...
Existing reviewers' "Open questions" sections are informational — the controller may or may not address them. **A subset of Extension architect's open questions — those tagged architectural-intent — are blocking-by-default**: each tagged question requires an explicit substantive disposition in the respondent post for the round before re-review can verify it as addressed.

Per red-team Opus's review of this PR: the blocking-by-default property attaches to a CLASS of question (architectural intent — "did you mean to commit to X by saying Y?"), NOT to all of Extension architect's open questions. Other extension-architect open questions (e.g., minor clarification, future-iteration suggestions, devil's-advocate flags) remain informational. The role's prompt template defines the convention: the reviewer marks blocking questions explicitly (e.g., with **bold-leading-text** or a `[blocking]` prefix) in the "Open questions" section. The controller's respondent then knows which ones need substantive dispositions vs which can use bare-noted forms.

The architectural-intent vs informational distinction matters because Extension architect's mandate is architecture-correctness, but the reviewer may surface broader open questions (UX adjacents, plausibly-related concerns) that don't rise to "must answer before merge." Forcing substantive dispositions on every question would create disposition-theater; reserving the discipline for architectural-intent questions preserves its signal.
...
**Re-review obligation:** Extension architect's re-review specifically checks that each prior question got a substantive disposition. Silent dispositions get re-flagged. This becomes a tripwire-class check (see Tripwires).

The blocking-by-default discipline is **specific to Extension architect's "Open questions"** in v1, not generalized to all reviewers' open questions. Generalizing is a future iteration if needed.

### Operational prerequisite — `gcscode-extension-architect` GitHub App
...
2. **Name:** `gcscode-extension-architect` (exactly — determines the rendered `[bot]` username)
3. **Description:** "Extension-architect reviewer for gcscode (extension architecture). See `docs/specs/2026-05-17-reviewer-routing-and-extension-architect.md`."
...
10. Generate + download private key; rename and move:
    ```
    mv ~/Downloads/gcscode-extension-architect.*.private-key.pem ~/.config/gcscode/gcscode-extension-architect.pem
    ```
...
```
extension-architect: appId=<...>, installationId=<...>
```

The PEM at `~/.config/gcscode/gcscode-extension-architect.pem` is found via the default-path fallback (per [`docs/specs/2026-05-17-robust-default-paths-in-token-helpers.md`](2026-05-17-robust-default-paths-in-token-helpers.md)) — no env var update needed.

### Helper script extension

`.claude/scripts/gh-app-token-reviewer` currently validates the role-slug enum: `spec-compliance|code-quality|final-review|red-team|spec-quality`. v1 extends this to include `extension-architect`. One-line change to the `case` statement + the validation error messages.

### Agent file + prompt template

Two new files:

- `.claude/agents/extension-architect-reviewer.md` — dispatch wrapper (model + effort frontmatter). Same shape as `red-team-reviewer.md` / `spec-quality-reviewer.md`. Model: Opus 4.7 (high-leverage, architectural-judgment role). Effort: max.
- `.claude/reviewer-prompts/extension-architect.md` — role prompt template with persona, scope, lens questions, output structure, posting block, header convention. Full content in Post-merge implementation > Commit 4.
...
- For Extension architect (heuristic-conditional): controller applies the heuristic, dispatches if any trigger matches, OR skips dispatch entirely if the heuristic indicates out-of-scope.
- Existing 3 roles continue to dispatch unconditionally (`Routing condition: always`).
- On `Code-review-followup:` commits, controller re-applies the heuristic — if a followup brings new extension-architecture content into scope, dispatch may newly fire for Extension architect even if it was skipped initially.

### Gate 3b unaffected

The auto-merge workflow's Gate 3b (`gcscode-red-team count >= 2 + gcscode-spec-quality count >= 1`) is **unchanged**. Extension architect is advisory and out of the gate. Its skip-or-substantive-review status doesn't affect merge readiness. The user can manually intervene (don't apply `auto-merge` label) if Extension architect raises blocking concerns.
...
- **PRs opened pre-merge that complete post-merge**: Extension architect is NOT dispatched on them retroactively. The role didn't exist when the PR was reviewed; introducing it mid-flight would re-trigger reviewer rounds on PRs the user has already cognitively closed. Treated as a one-time cliff: pre-iteration PRs miss out on extension-architect coverage; post-iteration PRs get it.
- **The first post-iteration spec/ADR PR** is where the routing-layer + extension-architect behavior is exercised live. The controller applies the heuristic for the first time AND verifies the auto-dispatch obligation correctly includes Extension architect (if in-scope) or correctly skips it (if out-of-scope).
- **No retroactive scope-evaluation**: even if a pre-merge PR's content matches the heuristic in hindsight, Extension architect is not dispatched on it. The cliff is acceptable because the cost of retroactive dispatch (re-triggering reviewer rounds, re-running respondents on stale dispositions) exceeds the value (one missed review on a PR the user already closed).
...
- **Validation by review on this PR.** Reviewers (red-team Opus + Sonnet + spec-quality) verify the routing-layer architecture, the extension-architect persona/scope, the heuristic shape, the escape valve mechanics, and the blocking-question discipline. Note: Extension architect itself does NOT review this PR (it doesn't exist yet at review time AND its heuristic would correctly identify "this is about reviewer routing + agentic team, not extension architecture" — out of scope).
- **Validation by use on the FIRST spec/ADR PR after merge that touches extension architecture.** The next real extension-architecture spec/ADR exercises the heuristic dispatch + the Extension architect's actual review. Plan 1 below.
...
### Plan 1: Extension architect dispatches and reviews substantively

The first real spec/ADR PR after merge that touches extension architecture (heuristic returns "in scope"):

- Controller dispatches Extension architect per the auto-dispatch obligation
- Extension architect produces a 4-section review (Premises / Drift / Open questions / Summary)
- Extension architect's review posts under `gcscode-extension-architect[bot]`
- If Extension architect raises "Open questions," the controller's respondent post substantively dispositions each (no silent "we'll consider it")
- Re-review (if any) verifies the dispositions

Disposition: keep open as reference artifact if Extension architect's first live review surfaces something material that informs future iterations of the role.

### Plan 2: Extension architect correctly skips dispatch

The first real spec/ADR PR after merge that does NOT touch extension architecture:

- Controller evaluates the heuristic, determines out-of-scope, does NOT dispatch Extension architect
- No `gcscode-extension-architect[bot]` post appears on the PR
- The PR's review timeline contains only red-team Opus + Sonnet + spec-quality (the always-dispatch roles)

This validates the heuristic's skip logic. If the controller dispatches Extension architect on a clearly-out-of-scope PR by mistake, the reviewer can use the escape valve to return a skip — that ALSO validates the escape valve mechanics.

### Plan 3: Blocking-question discipline holds

After plan 1 (or any subsequent extension-architecture PR), verify:

- Extension architect's "Open questions" section, if non-empty, gets per-question substantive dispositions in respondent post
...
Extension architect's persona is VSCode/Atom/Eclipse-flavored. gcscode is explicitly "VS Code-style" per `shell/CLAUDE.md` ("Layout: pnpm workspace with three packages, scaffolded as the shell for a VS Code-style ground control station"). The persona's reference points match gcscode's architectural anchors. This is consistent VS Code alignment, not divergence.
...
- **Self-triage abuse**: Extension architect always defers via the escape valve (never produces a substantive review across N=3 PRs). v1 tripwire below.
- **Silent-disposition pattern on blocking questions**: controller's respondent uses bare `noted, no current action — <generic>` without inline answer on Extension architect's tagged-blocking open questions. v1 tripwire below.
...
- **Gate 3b unchanged but Extension architect visible**: a future reader may expect Extension architect in the gate. The CLAUDE.md text explicitly notes Extension architect is advisory + out-of-gate to prevent confusion.

## Tripwires for known-quality concerns

- **Heuristic false-negative tripwire**: if a spec/ADR PR ships that retrospectively should have triggered Extension architect dispatch (extension-architecture content present but no extension-architect review on the PR), AND the omission is discovered post-merge (e.g., during a later iteration's review that catches an architectural issue Extension architect would have caught), the heuristic missed a real case. **Fires at N=1** for clear cases (Extension architect would unambiguously have had something to say); responses: tighten the heuristic + add the missed signal class to the auto-dispatch obligation. Per CLAUDE.md tripwire condition (iii) binary-rule carve-out: this is grep-able post-hoc from PR archives.
- **Self-triage abuse tripwire**: if Extension architect returns "out-of-scope skip" on **N=3 consecutive spec/ADR PRs** where it WAS dispatched (and across at least 1 PR the heuristic was clearly correct to dispatch), the role's threshold for engaging is too high. Response: sharpen the prompt template's "do not punt; engage if any of your lens applies" framing.
- **Silent-disposition pattern tripwire**: if controller's respondent posts use bare `noted, no current action — <generic placeholder>` on Extension architect's tagged-blocking open questions across **N=2 spec/ADR PRs**, the blocking-question discipline isn't holding. Response: revise the respondent prompt template to reject silent dispositions for questions tagged blocking-by-default (the disposition vocabulary becomes stricter for these).
- **Architectural-intent tagging failure-mode tripwire (NEW, per Opus's round-2 re-review)**: the architectural-intent tagging mechanic has two failure modes. **Tag-zero**: if Extension architect produces a substantive review with ANY open questions but tags ZERO as architectural-intent across **N=3 spec/ADR PRs** that contain non-trivial extension-architecture content, the reviewer is avoiding accountability for real architectural questions. **Tag-everything**: if Extension architect tags ALL open questions as architectural-intent across **N=2 spec/ADR PRs**, the reviewer is restoring the original blocking-all-questions problem the architectural-intent narrowing was meant to fix. Response (tag-zero): sharpen the prompt template's framing on what qualifies as architectural-intent and add explicit examples. Response (tag-everything): clarify the CLASS boundary in the prompt template + add the disposition-theater failure mode as an explicit warning. Both tripwires are pattern-across-N; both align with CLAUDE.md condition (iii).

Tripwires are manual review items, not automated checks; controller observes from the PR timeline.

## Future iterations

1. **Verdict promotion for Extension architect** (`--request-changes` blocking). Trigger: advisory mandate insufficient (a blocking-concern is repeatedly ignored). Designs the override path under auto-merge.
...
4. **Generalized blocking-question discipline**. Trigger: other reviewers' open questions also start accumulating silent dispositions. Extends the blocking-by-default + substantive-disposition rule beyond Extension architect.
...
Surfaced 2026-05-17, immediately after the multi-model red-team v1 evaluation concluded KEEP-BOTH (based on observed independence-of-opinion across PRs #18, #20, #21, #22, #23). The user asked whether adding an Extension architect reviewer was feasible. Feasibility check confirmed yes — the agentic-actor registry (ADR-0009) was built as a registry precisely so new reviewer roles add additively. The user provided a brainstorm draft from an earlier Claude session; this iteration tightens that draft to match the current architecture's conventions (4-section output, respondent-based question handling, registry-driven routing).
...
Plus 2 ad-hoc operational iterations (relative-paths, robust-default-paths) and this iteration's extension-architect + routing combination. The debt-clearing v1 queue is fully drained as of this iteration shipping.
...
- **Commit 1:** User creates `gcscode-extension-architect` GitHub App out-of-band (operational prerequisite); controller updates `.claude/agent-config.json` to add `extension-architect` sub-object under `reviewerApps`.
- **Commit 2:** Extend `.claude/scripts/gh-app-token-reviewer` role-slug enum to include `extension-architect`.
- **Commit 3:** Create `.claude/agents/extension-architect-reviewer.md` (agent file with model + effort frontmatter).
- **Commit 4:** Create `.claude/reviewer-prompts/extension-architect.md` (prompt template with persona, scope, lens questions, 4-section output structure, posting block).
- **Commit 5:** CLAUDE.md edits (seven sub-edits, 5a-5g). Registry table (new column `Routing condition` + new row for Extension architect), verdict-permission table (new row for Extension architect), "Auto-dispatch on spec/ADR PRs" subsection update, PR-template footer update (drops "Extension architect, when they exist"), two new subsections (Routing layer discipline embedding the Out-of-scope skip pattern; Blocking-question discipline).
...
After the user creates the App and provides `appId` + `installationId`, replace the contents of `.claude/agent-config.json` with the following structure, adding a sixth `extension-architect` sub-object under `reviewerApps`:
...
The 5 existing `reviewerApps` entries are unchanged. The `respondentApp` entry is unchanged. Only the new `extension-architect` sub-object is added.

### Verbatim — Commit 2 (`.claude/scripts/gh-app-token-reviewer` enum extension)

In `.claude/scripts/gh-app-token-reviewer`, locate the role-slug validation block and extend it to include `extension-architect`:
...
```bash
case "$role_slug" in
  spec-compliance|code-quality|final-review|red-team|spec-quality|extension-architect) ;;
  *)
    echo "gh-app-token-reviewer: unknown role-slug: $role_slug" >&2
    echo "gh-app-token-reviewer: valid role-slugs: spec-compliance, code-quality, final-review, red-team, spec-quality, extension-architect" >&2
    exit 1
    ;;
esac
```
...
No other changes to the script. The default-path fallback (per `robust-default-paths-in-token-helpers.md`) already resolves `$HOME/.config/gcscode/gcscode-extension-architect.pem` correctly once the PEM is in place.

### Verbatim — Commit 3 (`.claude/agents/extension-architect-reviewer.md` agent file)
...
### Verbatim — Commit 4 (`.claude/reviewer-prompts/extension-architect.md` prompt template)
...
**5b — Add Extension architect row to the registry.** Append a new row after the existing `Spec-quality` reviewer row (and before the respondent row). The row's column values (in column order):

- Actor class: `reviewer`
- Role: `Extension architect`
- Kind: `per-artifact`
- Identity: `` `gcscode-extension-architect[bot]` ``
...
- Header: `` `## Extension-architect review — <spec or ADR> — Claude Opus 4.7` ``
- Re-review header: `` `## Extension-architect review — <spec or ADR> (re-review of <SHA>) — Claude Opus 4.7` ``
- Prompt template: `` `.claude/reviewer-prompts/extension-architect.md` ``
...
Replace the existing paragraph beginning `**Auto-dispatch on spec/ADR PRs.**` with the following (preserves all existing content; adds the routing-evaluation step + Extension architect dispatch):
...
> - **Heuristic-conditional roles** (Routing condition: `heuristic: <description>`) — Extension architect. The controller evaluates the heuristic against the PR's content (file paths, spec/ADR text, ADR citations, VS Code alignment section). For Extension architect specifically: dispatch if the PR touches extension-architecture concerns (extension API surface, host↔extension boundary, lifecycle, isolation, IPC, discovery, conflicts, performance — see [`docs/specs/2026-05-17-reviewer-routing-and-extension-architect.md`](docs/specs/2026-05-17-reviewer-routing-and-extension-architect.md) Architecture > Extension architect's heuristic for the concrete signal list). If the heuristic indicates out-of-scope, skip dispatch entirely (no review posted, not counted in any gate).
>
> When in scope, the controller dispatches: red-team Opus 4.7 (primary), red-team Sonnet 4.6 (secondary, per multi-model dispatch), spec-quality Sonnet 4.6, AND Extension architect Opus 4.7 (if heuristic indicates in-scope). Up to 4 parallel subagent dispatches. Dispatch identifiers: `subagent_type: red-team-reviewer` for primary Opus dispatch, `subagent_type: red-team-reviewer, model: sonnet` for secondary, `subagent_type: spec-quality-reviewer` for spec-quality, `subagent_type: extension-architect-reviewer` for Extension architect. The subagents dispatch as independent calls; none blocks any other; each posts an independent review under its per-role identity.
>
> The two red-team dispatches use the same prompt template (`.claude/reviewer-prompts/red-team.md`); only the `model` parameter differs. All subagents run with `effort: max` per the `effort` field in their agent files (`.claude/agents/red-team-reviewer.md`, `.claude/agents/spec-quality-reviewer.md`, `.claude/agents/extension-architect-reviewer.md`). All verdicts are `--comment` only in v1 (advisory). On a `Code-review-followup:` commit, the controller re-evaluates the routing condition for heuristic-conditional roles (a followup may newly bring extension-architecture content into scope) and re-dispatches the same set as initial dispatch (plus newly-routable roles). Each re-review header includes `(re-review of <SHA>)` where `<SHA>` is the followup commit.
...
>
> - `always` — dispatch unconditionally when the trigger fires. All v1 reviewer + respondent roles default to this except Extension architect.
> - `heuristic: <one-line description>` — dispatch conditionally. The controller evaluates the heuristic against PR content. Extension architect is the only `heuristic`-conditional role in v1.
>
...
>
> Controller treats a skip as a no-op: doesn't trigger followup loop, doesn't count in any Gate, doesn't request re-review on subsequent commits in the same PR's lifecycle — **except** when a `Code-review-followup:` commit materially changes scope (e.g., adds extension-architecture content not present at initial-review time), in which case the controller may re-evaluate the heuristic and re-dispatch. This is consistent with the heuristic-re-evaluation step in "Auto-dispatch on spec/ADR PRs" (above). The skip post stays on the PR as a record of the reviewer's determination at the time it was made.
>
> Spec: [`docs/specs/2026-05-17-reviewer-routing-and-extension-architect.md`](docs/specs/2026-05-17-reviewer-routing-and-extension-architect.md).

**5e — Add "Blocking-question discipline" subsection.** Insert immediately after the "Routing layer discipline" subsection:

> **Blocking-question discipline (Extension architect, architectural-intent questions only).** A subset of Extension architect's "Open questions" findings — those tagged architectural-intent (the reviewer marks them explicitly, typically with a `[blocking]` prefix or **bold-leading-text**) — are **blocking-by-default**: each tagged question requires an explicit substantive disposition in the round's respondent post before re-review can verify the question as addressed. Other extension-architect open questions (minor clarifications, future-iteration suggestions, devil's-advocate flags) remain informational. This applies specifically to Extension architect in v1; existing reviewers' open questions remain informational.
>
...
> - `routed to docs/roadmap.md / out-of-scope.md` — for future-iteration questions.
>
> Extension architect's re-review checks each tagged-blocking question for substantive disposition; silent dispositions are re-flagged. Spec: [`docs/specs/2026-05-17-reviewer-routing-and-extension-architect.md`](docs/specs/2026-05-17-reviewer-routing-and-extension-architect.md).

**5f — Add Extension architect row to the verdict-permission table.** The verdict-permission table currently has 6 rows (Per-task spec-compliance, Per-task code-quality, Final cross-cutting, Red-team, Spec-quality, Respondent). Add a new row immediately after the Spec-quality row (before Respondent):
...
> Red-team auto-dispatches on PR open per the agentic-actor registry. Future reviewer roles (e.g., Extension architect, when they exist) follow per the registry.

Replace with:

> Red-team and Extension architect (heuristic-routed) auto-dispatch on PR open per the agentic-actor registry. Spec-quality auto-dispatches unconditionally. Future reviewer roles follow per the registry.
...
```md
- [ ] **Reviewer routing layer** — long-standing Considering item; "which reviewer roles fire on which PRs." Becomes load-bearing when a 4th reviewer role arrives (Extension architect, devil's-advocate v2, etc.). Trigger: 4th reviewer role is added.
```
...
```md
- [x] **Reviewer routing layer + Extension architect (combined)** — closes queued #4 (routing layer) AND ships the first reviewer role that uses routing (Extension architect on extension architecture). Agentic-actor registry gains `Routing condition` column (`always` default for existing roles; `heuristic: <description>` for new heuristic-conditional roles). Controller auto-dispatch obligation extended to evaluate routing condition before dispatching. Out-of-scope skip exit pattern (`## <Role> review — out-of-scope skip — <model>`) for heuristic-conditional roles when controller's heuristic was wrong. Extension architect persona: principal-level architect on extensible hosts (VSCode/Atom/Eclipse-flavored + gcscode-specific awareness, free to push back on past architectural commitments). Output format aligns to existing 4-section convention. Blocking-question discipline: Extension architect's open questions require substantive dispositions (silent `we'll consider it` insufficient). Gate 3b unchanged (Extension architect is advisory, out of gate). New `gcscode-extension-architect` GitHub App; appId/installationId added to `reviewerApps` config. Blocking-question discipline scoped to architectural-intent questions only (tagged by reviewer with `[blocking]` prefix or bold-leading-text). Four tripwires: heuristic-false-negative + self-triage abuse + silent-disposition pattern + architectural-intent tagging failure-mode (tag-zero / tag-everything). Spec: [`specs/2026-05-17-reviewer-routing-and-extension-architect.md`](specs/2026-05-17-reviewer-routing-and-extension-architect.md).
```
